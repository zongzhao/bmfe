...
exports.pWriteFile = exports.pMkdir = exports.readJSONSync = undefined;
...
var _mkdirp = __webpack_require__(15);
...
// 读取 json
var readJSONSync = exports.readJSONSync = function readJSONSync(filename) {
  try {
    var filedata = _fs2.default.readFileSync(filename);
    if (filedata) {
...
// 创建目录
var pMkdir = exports.pMkdir = function pMkdir(dir) {
  if (!_fs2.default.existsSync(dir)) {
    _fs2.default.mkdir(dir, function (err) {
      if (err) {
        console.log('Make ' + dir + ' ... ' + _chalk2.default.red(err.message));
      } else {
        console.log('Make ' + dir + ' ... ' + _chalk2.default.green('OK'));
      }
    });
  } else {
    console.log('Make ' + dir + ' ... ' + _chalk2.default.green('OK'));
...
// 写文件
var pWriteFile = exports.pWriteFile = function pWriteFile(filename, retmpl) {
  var tmpl = _fs2.default.readFileSync(retmpl);
  var dir = _path2.default.dirname(filename);

  if (dir) {
    (0, _mkdirp2.default)(dir, function (err) {
      if (err) {
        console.log('Write ' + filename + ' ... ' + _chalk2.default.red(err.message));
        return;
      }

      _fs2.default.writeFile(filename, tmpl, function (err) {
        if (err) {
          console.log('Write ' + filename + ' ... ' + _chalk2.default.red(err.message));
        } else {
          console.log('Write ' + filename + ' ... ' + _chalk2.default.green('OK'));
        }
      });
    });
    return;
...
module.exports = {
	"name": "bmfe",
	"version": "0.1.1",
	"description": "banma fe assistant",
	"main": "dist/bmfe.js",
	"bin": {
		"bmfe": "bin/bmfe"
	},
	"scripts": {
		"start": "node ./bin/bmfe",
		"test": "webpack --progress --watch & npm start",
...
// 命令根目录
var dirname = _path2.default.join(__dirname, '..', 'src/commands');

var run = function run(command) {
  var repo = command.name();
  var options = command.opts();

  var _cmd = (0, _fs3.readJSONSync)(_path2.default.join(dirname, repo, 'command.json'));

  if (_cmd) {
    if (_cmd.installed === false) {
      console.log(_chalk2.default.red('\u6B64\u547D\u4EE4\u9700\u8981\u5B89\u88C5\u540E\u624D\u80FD\u4F7F\u7528\uFF0C\u8BF7\u8FD0\u884C bmfe install --cmd ' + repo + ' \u8FDB\u884C\u5B89\u88C5\u3002'));
      process.exit(1);
    }

    if (_cmd.requires && _cmd.requires.length) {
      _cmd.requires.some(function (_repo) {
        var _stats = _fs2.default.existsSync(_path2.default.join(dirname, _repo, 'command.json'));
        if (!_stats) {
          console.log(_chalk2.default.yellow.bold('\u76D1\u6D4B\u5230 ' + repo + ' \u6240\u4F9D\u8D56\u7684 bmfe \u547D\u4EE4 ' + _repo + ' \u6709\u8BEF\uFF0C\u53EF\u80FD\u5BFC\u81F4 ' + repo + ' \u4E0D\u80FD\u4F7F\u7528\u3002'));
        }
      });
    }
  }

  __webpack_require__(9)("./" + repo + '/run').then(function (run) {
    run.default(options);
  }).catch(function (err) {
    console.log(_chalk2.default.red(repo + '\u6267\u884C\u5931\u8D25'));
  });
};
...
// 定义使用方法
_commander2.default.usage('<command>');

_commander2.default.command('help [cmd]').description('显示命令 [cmd] 的帮助').action(function (cmd) {
  _commander2.default.commands.some(function (command) {
...
var files = _fs2.default.readdirSync(dirname);

_async2.default.each(files, function (repo, callback) {
  var _cmd = (0, _fs3.readJSONSync)(_path2.default.join(dirname, repo, 'command.json'));

  if (_cmd) {
    var _pro = _commander2.default.command(_cmd.name).description(_cmd.description);
